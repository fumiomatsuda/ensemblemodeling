...
if __name__ == '__main__':
    ...
    import itertools
    cpus = 23
    filenamestem = "200325"   #ファイル名の語幹
    parallel = 1000#100
    repeat = 500#元々は2
    #
    # [S]mixo data
    #
    metabolites_dic = {"2PG": 13.46,"3PG": 36.05,"6PG": 1.34, "AcCoA": 1.36,"ATP": 0.93,"citrate": 2.08,"isoCitrate": 3.65,
    "DHAP": 0.22,"G6P": 2.22,"F6P": 0.86,"FBP": 0.18,
    "PEP": 12.35,"Ru5P": 0.03,"R5P": 0.03,"S7P": 0.008,"RuBP": 0.001,
    "Succinate": 0.75,"Malate": 0.47,"NADP": 0.8,"NAD": 0.14}

    effector_list=['3PG','PEP','2PG','2KG','citrate','NADP','G6P','AcCoA','ATP','PYR','RuBP','isoCitrate', 'ADP', 'NADPH']
    reaction_list=['prk', "gpm", "pyk",'rbc','pgk','zwf']
    ...
    for (reaction, effector) in itertools.product(reaction_list,effector_list):
        ...
        for i in range(100):
            ...
            #
            # Add allosteric inhibition to model definition data
            #
            reactions_copy= copy.deepcopy(reactions)
            if  reactions_copy[reaction]["effector"] == "":
                reactions_copy[reaction]["effector"] = effector
            else:
                reactions_copy[reaction]["effector"] = reactions_copy[reaction]["effector"]+"+"+effector

            reactions_copy[reaction]['type'] = reactions_copy[reaction]['type']+"_i"
            #
            # Construct a instances of EnsembleModel indluing 1000 models
            #
            emodel = EnsembleModel(metabolites,reactions_copy, parameters, parallel, numberofcpus = cpus)
            #
            # Parameter sampling
            #
            ...
            print("New models are constructed."+reaction+effector+str(counter)+" "+str(i))
            #
            # Solve to check the stability
            #
            emodel.odeint_parallel(time = 10000, step = 10)
            print("Initial state checked"+str(counter))
            ...
            for j in range(len(emodel.models)):
                ...
                if emodel.model_use[j] == "use":
                    ...
                    rss_list.append(rss)
                    rss_log = reaction+"\t"+effector+"\t"+str(j)+"\t"+str(rss)+"\n"
                    with open("validationlog.txt", "a") as file:
                        file.write(rss_log)
                    print(rss_log)

        with open("validationlog.txt", "a") as file:
            text = reaction+"\t"+effector+"\t"+str(counter)+"\n"
            file.write(text)
